    ...
    keyResponsibilities: [
      {
        ...
        problem: '고객이 자원을 직접 제어할 수 없고 운영자가 대신 생성해주는 매니지드 방식이라, 방화벽·포트포워딩 변경마다 요청·대기가 반복(체감상 월 50건 안팎)되는 제한적인 CSP 구조',
        analyze: '네트워크 장비가 이미 제공하는 멀티테넌트 기능을 외부에 안전하게 노출하는 별도 API 서버를 신규 개발 → 고객이 자기 테넌트 범위에서 직접 자원을 신청·즉시 처리',
      },
      {
        result: '다단계 벤더 프로비저닝을 **Saga 패턴(11개 예외 타입 기반 차등 롤백)**으로 설계 · 장비 응답 실패에 한해 자원 고아 상태 구조적 방지, 벤더 추가 시 예외 타입·핸들러만 추가하는 확장 구조',
      ...
      {
        result: '로드밸런서 공유 자원(Citrix Service)에 **참조 카운팅**을 설계 · 상태 없는 벤더 API 위에서 여러 가상서버의 백엔드 공유를 안전하게 보장, 중복 생성·조기 삭제 방지',
        problem: 'Citrix 장비에서 백엔드는 {IP}_{PORT}로 식별되는 하나의 Service 객체인데 여러 로드밸런서(가상서버)가 같은 Service를 공유 바인딩 가능. 장비 자체엔 참조 카운트 개념이 없어 중복 생성·사용 중인 자원의 조기 삭제 위험',
