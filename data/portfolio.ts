...
export const portfolioProjects: PortfolioProject[] = [
  {
    ...
    keyResponsibilities: [
      ...
      {
        result: '다단계 벤더 프로비저닝을 **Saga 패턴(11개 예외 타입 기반 차등 롤백)**으로 설계 · 자원 고아 상태 구조적 방지(장비 정상 응답 실패 기준), 벤더 추가 시 예외 타입·핸들러만 추가하는 확장 구조',
      ...
      },
      {
        ...
        problem: 'Citrix 장비에서 백엔드는 {IP}_{PORT}로 식별되는 하나의 Service 객체인데 여러 로드밸런서(가상서버)가 같은 Service를 공유 바인딩 가능. 여러 가상서버가 안전하게 공유하려면 참조 상태를 애플리케이션에서 직접 파악해야 해 중복 생성·사용 중인 자원의 조기 삭제 위험',
