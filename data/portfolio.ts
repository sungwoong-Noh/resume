...
export const portfolioProjects: PortfolioProject[] = [
  {
    ...
    detailParagraphs: [
      ...
      '**방화벽**은 Zone(테넌트)을 루트로 하는 FK 그래프로 설계했습니다. zone_idx(Zone PK)를 경계 루트인 FireWallVs에만 두고, 하위 엔티티는 상위로 조인해 테넌트를 식별하도록 정규화했습니다. **로드밸런서**는 LoadbalancerEntity에 zone_idx(DB 식별자)와 lb_partition(장비 Citrix의 Partition 이름)을 나란히 저장하는 비정규화로 설계했습니다. DB 조회와 장비 API 호출에 각각 맞는 키를 바로 쓸 수 있게 한 것입니다. 여기에 DB는 FK 논리 격리, 물리 장비는 TrusGuard의 VS·Citrix의 Partition으로 테넌트별 물리 격리하는 **2단 구조**를 구성했습니다.',
    ],
    keyResponsibilities: [
      {
        result: '운영자가 대신 처리하던 매니지드 방식을 **고객 셀프서비스 API로 전환** · 반복적인 수동 처리 업무 해소',
      ...
      {
        result: '다단계 벤더 프로비저닝을 **Saga 패턴(11개 예외 타입 기반 차등 롤백)**으로 설계 · 자원 누수 구조적 방지, 벤더 추가 시 예외 타입·핸들러만 추가하는 확장 구조',
        problem: '방화벽·VS·라우트·ACI·OpenStack 등 서로 다른 벤더 API의 순차 호출이 외부 시스템이라 DB 트랜잭션·@Transactional로 묶을 수 없어, 중간 실패 시 자원 누수가 발생하고 재시도조차 막힘',
        analyze: '벤더 호출을 조율하는 Integration 계층 분리(도메인 서비스 / 벤더 어댑터 / Builder) + 프로비저닝 7단계마다 실패 지점별 롤백 범위가 다른 11개 예외 타입 체계 설계(공통 예외 상속, 하위 예외는 컨텍스트만 추가) · 도메인별 구체 롤백은 각 도메인 담당자가 이 구조 위에서 처리',
      },
      {
        result: '로드밸런서 공유 자원(Citrix Service)에 **참조 카운팅**을 설계 · 애플리케이션 레벨에서 참조 상태를 직접 관리해 여러 가상서버의 백엔드 공유를 안전하게 보장, 중복 생성·조기 삭제 방지',
        problem: 'Citrix 장비에서 백엔드는 {IP}_{PORT}로 식별되는 하나의 Service 객체인데 여러 가상서버가 같은 Service를 공유 바인딩 가능. 여러 가상서버가 안전하게 공유하려면 애플리케이션이 직접 참조 상태를 추적해야 해 중복 생성·사용 중인 자원의 조기 삭제 위험',
      ...
      {
        result: '장비 API를 실제 호출하는 **통합 테스트**로 스펙 불일치를 배포 전 검증 · 최전방 네트워크 설정 실패로 인한 테넌트 전체 연결 장애 사전 차단',
        ...
        analyze: '벤더 장비 연동을 검증하는 테스트 문화가 없던 조직에서, Mock 없이 실제 스테이징 장비를 직접 호출하는 통합 테스트를 도입 — 비즈니스 로직보다 요청 포맷이 실제 장비 스펙과 어긋나지 않는지를 배포 전 검증하는 데 초점',
      },
    ],
    techStack: {
...
export const portfolioOtherProjects: PortfolioProject[] = [
  {
    ...
    detailParagraphs: [
      ...
      'JSP 화면마다 헤더·푸터·네비게이션을 반복 복붙하는 구조라 레이아웃 변경 시 전 페이지를 손봐야 했습니다. include 방식은 중복 제거 효과가 제한적이라 **레이아웃 템플릿화가 가능한 Tiles 프레임워크 도입을 직접 제안·적용**했습니다. 중복 코드를 65%에서 10%로 줄였습니다.',
      '폐쇄망·온프레미스 환경이라 관리형 CI를 쓸 수 없었습니다. **자체 구축 가능한 Jenkins로 배포 파이프라인을 구성**해 배포 주기를 7일에서 1일로 줄였습니다.',
    ],
    keyResponsibilities: [
      {
...
export const portfolioProfile = {
  ...
  introduction: [
    '문제를 만나면 표면이 아니라 원인을 봅니다. 14만 건 배치가 2시간 걸리던 원인을 JPA IDENTITY의 Bulk Insert 한계까지 파고들어 10초로 줄였습니다.',
    'PPP Cloud에서는 트랜잭션으로 묶을 수 없는 다단계 벤더 프로비저닝을 Saga 패턴(11개 예외 타입 기반 차등 롤백)으로 설계하고, 상품 도메인별 특성에 맞춘 멀티테넌트 데이터 모델을 만들었습니다. RPMS에서는 공공 예산심사의 감사·심사 규정을 diff 기반 이력 관리와 Bean Validation Group으로 도메인 모델에 정확히 옮겼습니다.',
...
export const portfolioSideProjects: PortfolioSideProject[] = [
  {
    ...
    detailParagraphs: [
      ...
      '**AWS ECS 기반 Auto-scaling 구조**를 직접 설계해, 급격한 트래픽 변동에도 가용성을 유지하도록 했습니다. 분산 환경의 인증 문제는 **API Gateway 중심의 JWT 무상태(Stateless) 인증 체계**로 풀어 서비스 간 결합도를 낮췄습니다.',
