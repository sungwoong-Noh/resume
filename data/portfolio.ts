...
export const portfolioProjects: PortfolioProject[] = [
  {
    ...
    keyResponsibilities: [
      ...
      },
      {
        ...
        problem: 'Citrix 장비에서 백엔드는 {IP}_{PORT}로 식별되는 하나의 Service 객체인데 여러 로드밸런서(가상서버)가 같은 Service를 공유 바인딩 가능. 여러 가상서버가 안전하게 공유하려면 애플리케이션이 직접 참조 상태를 추적해야 해 중복 생성·사용 중인 자원의 조기 삭제 위험',
