        ...
        resumeContent: [
          {
            ...
            body: [
              'ITSM에서 국정자원 공무원들이 자산을 조회하는 기능으로, **ID만으로 검색하면 빠른데 검색 조건을 추가하면 응답이 급격히 느려지는** 문제가 있었습니다. **130만 건 규모에서 p95가 17초**에 달해, 실무자들이 조건을 걸어 자산을 찾을 때마다 매번 십수 초씩 멈춰 업무가 지연됐습니다.',
              '실행계획을 분석해 **WHERE절 컬럼 타입과 파라미터 타입이 불일치**해(조건 추가 시) **인덱스가 무력화**되고 있다는 원인을 파악하고, 타입 정합성을 수정했습니다.',
            ],
