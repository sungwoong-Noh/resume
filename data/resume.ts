...
export const experiences: Experience[] = [
        ...
        resumeContent: [
          {
            ...
            body: [
              'MSP로 관리하는 **40개 이상 고객사의 클라우드 인프라 통합 관제 체계**를 제안부터 전체 구축까지 **단독으로 수행**한 프로젝트입니다. 기존 KT Cloud Watch는 기본 자원 지표를 **7일만 보존**해 일주일 이상 지난 장애의 원인 추적이 불가능했고, **40개 고객사를 한눈에 보는 통합 관제도 없었습니다**. 문제가 생기면 수십 개 서버를 일일이 접속해 로그를 뒤져야 했고, 월간 보고서도 서버마다 직접 접속해 리소스를 확인하는 방식이라 **서버 20개 기준 2시간 이상** 걸렸습니다.',
              '이를 해결하기 위해 모니터링 스택 도입을 검토했습니다. **시각화와 알람 연동을 단일 도구로 처리할 수 있는 Grafana를 선택**했고, **메트릭 수집은 Prometheus로 구성**했습니다.',
              '다만 고객사 모니터링 서버 사양이 제한적이어서 Prometheus 단독으로 장기 보존하면 용량 부족 위험이 있었습니다. Thanos 등 별도 장기 저장 구성을 검토한 뒤, **압축 효율이 높고 인프라 변경 없이 도입 가능한 시계열 DB VictoriaMetrics를 적용해 1년 치 메트릭을 안정적으로 보존**할 수 있도록 했습니다.',
            ...
            result: '40개 고객사 **통합 관제 대시보드 구축** · 지표 보존 **30일 → 1년** (VictoriaMetrics 전환) · 월간 보고서 작성 시간: **2시간 → 10분** (서버 20개 기준)',
          },
        ],
        achievements: [
          {
            problem: 'MSP로 관리하는 **40개 이상 고객사**의 지표가 KT Cloud Watch에 **7일만 보존**돼 장기 추세 분석·사후 원인 파악이 불가능했고, **40개를 한눈에 보는 통합 관제도 부재**. 월간 보고서도 서버마다 직접 접속해 리소스를 확인하는 방식으로 **서버 20개 기준 2시간 이상** 소요.',
            analyze: '메트릭 수집은 **Prometheus로 구성**하고 시각화·알람은 **Grafana로 통합**. 1년 보존이 필요했으나 Prometheus 단독은 디스크 부담이 커, Thanos 등을 검토한 뒤 **인프라 구성 변경 없이 도입 가능한 VictoriaMetrics로 전환해 1년치 메트릭을 안정적으로 보존**.',
            result: '40개 고객사 **통합 관제 대시보드 구축** · 지표 보존 **30일 → 1년** · **2시간 이상 걸리던 수기 보고서 작업이 10분으로 단축**',
          },
        ],
      },
      {
        ...
        resumeContent: [
          {
            ...
            body: [
              ...
              '**Gmail MCP 서버를 구성하고 Claude Code와 연동**하여, 명령 하나로 **메일 조회·라벨링·발신 기관 분류 및 차단 요청 초안 작성까지 자동으로 처리하는 파이프라인**을 단독으로 구축했습니다.',
              '단순 자동화를 넘어, 복잡한 기관별 담당 업체 분류 로직을 AI 지침에 내재화하되, **최종 발송 전에는 담당자가 검토·승인하는 구조**로 설계하여 시스템의 효율성과 운영의 안정성을 동시에 확보했습니다. 직접 운영으로 검증한 뒤 **팀에 전파해 함께 사용하는 워크플로우**로 만들었습니다.',
            ],
            result: '유해 IP 수집·분류·초안 작성 자동화 및 담당자 개입 최소화 · **단독 구축 후 팀 전파**',
          },
        ],
        achievements: [
          {
            problem: 'WAF에서 유해 IP 이벤트가 발생할 때마다 **담당자가 수동으로 메일을 확인하고 차단 요청을 작성하는 비효율** 존재',
            analyze: '**Gmail MCP 서버를 구성하고 Claude Code와 연동**하여, 명령 하나로 **메일 조회·라벨링·발신 기관 분류 및 차단 요청 초안 작성까지 자동으로 처리하는 파이프라인**을 단독 구축. 복잡한 기관별 담당 업체 분류 로직을 AI 지침에 내재화하되, **최종 발송 전에는 담당자가 검토·승인하는 구조**로 설계.',
            result: '유해 IP 수집·분류·초안 작성 자동화 및 담당자 개입 최소화 · **단독 구축 후 팀 전파**',
          },
        ],
      },
  ...
  },
  {
    ...
    careerDescription: '자산계획 시스템(RPMS) 5인 팀 개발 및 운영 단독 전담, CSP 네트워크 상품(PPP Cloud) 로드밸런서·포트포워딩 개발 담당, 국정자원 3개 백오피스 시스템 통합 운영유지보수 수행',
    highlights: [
      '데이터 동기화 99% 개선 (2시간 → 10초)',
      '핵심 조회 p95 응답시간 82% 개선 (17초 → 3초)',
      'QueryDSL 도입 주도 · Native 쿼리 70% 제거',
      '운영 이슈 유형화·우선순위화로 인시던트 12건 → 5건/주 지속 감소',
      'RPMS 단독 운영·개발·고객 대응 전담 · ITSM 오류 개선·배포 참여',
      '폐쇄망 K8s 개발 클러스터·미들웨어 재구축 · Jenkins/Helm 배포 자동화',
    ],
    techStack: [
    ...
    projects: [
      {
        ...
        resumeContent: [
          {
            subtitle: '운영 이슈 유형화·우선순위화로 인시던트 12건 → 5건/주',
            body: [
              'ITSM·통합포털·RPMS 3개 시스템의 통합 운영유지보수에서 **RPMS 운영·개발·고객 대응을 단독 전담**하며, 현업 사용자의 요구를 직접 받아 정제하고 우선순위를 합의해 릴리스까지 이어갔습니다. 동일 오류가 반복 접수돼도 **즉각 패치 후 종결하는 관행**으로 근본 원인이 누적되어 인시던트가 줄지 않는 구조였습니다.',
              '월 30건 이상의 운영 이슈를 **유형별로 분류하고 재발 빈도로 우선순위화**해, 증상 패치 대신 **고빈도 원인부터 순차적으로 근본 원인을 제거**하는 방식으로 전환했습니다.',
            ],
            result: '**인시던트 12건 → 5건/주** (주간 접수 건수 직접 집계)',
          },
          {
        ...
        achievements: [
          {
            ...
            result: '**RPMS 단독 전담**으로 **대응 창구 단일화** · ITSM 개발·오류 개선 참여',
          },
          {
            problem: '**130만 건 규모의 자산 조회** 기능에서 검색 조건을 추가하면 응답이 급격히 느려지는 문제. **p95가 17초**에 달해 현업 업무 지연 발생.',
            analyze: '실행계획을 분석해 **WHERE절 컬럼 타입과 파라미터 타입이 불일치해 인덱스가 무력화**되고 있다는 원인을 파악, 타입 정합성 수정.',
            result: '자산 조회 응답속도 **p95 17초 → 3초** (82% 개선)',
          },
          {
            problem: '동일 오류가 반복 접수돼도 **즉각 패치 후 종결하는 관행**으로 **근본 원인이 누적**되어 인시던트가 줄지 않는 구조',
            analyze: '월 30건 이상의 운영 이슈를 **유형별로 분류**하고 **재발 빈도로 우선순위화**해, 증상 패치 대신 **고빈도 원인부터 순차적으로 근본 원인을 제거**하는 방식으로 전환',
            result: '**인시던트 12건 → 5건/주** (주간 접수 건수 직접 집계)',
          },
        ],
      },
        ...
        resumeContent: [
          {
            ...
            body: [
              '백엔드 5인 팀에서 **로드밸런서(L4)·포트포워딩 상품 개발을 담당**했습니다. 고객이 자원을 신청하면 운영자가 직접 내부 시스템에서 자원을 생성해야 하는 티켓 방식이었고, **반복 작업이 월 50건**에 달해 고객이 직접 자원을 제어할 수 없는 **반쪽짜리 CSP** 구조였습니다.',
              '**네트워크 장비가 멀티테넌트 기능을 제공한다는 점에 착안**해, 이를 외부에 API로 노출하는 별도 API 서버를 신규 개발했습니다.',
            ],
            result: '고객이 직접 자원을 신청하고 즉시 처리되는 구조로 전환, **수동 운영 티켓 월 50건 → 5건으로 축소**',
          },
          {
          ...
          },
          {
            ...
            body: [
              ...
              '선임 개발자와 함께 **코딩 컨벤션, 공통 모듈, 에러 처리 표준을 수립**해 구조를 정비했고, **JUnit5 단위테스트를 핵심 기능부터 점진적으로 도입**해 수동 검증 문화를 바꿨습니다.',
            ],
        ...
        achievements: [
          {
            ...
            result: '선임 개발자와 함께 **코딩 컨벤션·공통 모듈·에러 처리 표준을 수립**해 팀 전체에 적용',
          },
          {
            problem: '클라우드 상품 신청 시 공인IP·방화벽·Cisco 스위치·OpenStack 등 **다단계 장비 설정이 순차 실행**되는 구조에서 **중간 단계 실패 시 이전 설정이 그대로 남아 충돌** 발생',
            analyze: '장비 API와 Spring 트랜잭션이 서로 다른 도메인이라 **2PC(2-Phase Commit) 적용 불가** → 각 단계마다 실패 시 이전 상태로 되돌리는 **보상 로직을 직접 구현하는 Saga 패턴** 선택',
            result: '이후 **장비 설정 충돌 오류 발생 0건** 달성',
          },
          {
            problem: '고객이 자원을 신청하면 운영자가 직접 내부 시스템에서 자원을 생성해야 하는 티켓 방식. **반복 작업이 월 50건**에 달하고, 고객이 직접 자원을 제어할 수 없는 **반쪽짜리 CSP** 구조',
          ...
          },
          {
            ...
            analyze: '**핵심 기능에 점진적 적용을 제안·합의**해 **JUnit5 단위테스트 문화 정착** 유도',
            result: '**설정오류 0건** · **장비 API 스펙 불일치·IP 충돌 검증을 테스트 코드로 대체**해 서버 재실행 반복 제거',
          },
        ],
      },
      {
        ...
        resumeContent: [
          {
            ...
            body: [
              'PL·선임 개발자·신입 2명과 함께한 **5인 팀에서 핵심 기능인 자산 동기화를 담당**했습니다. 인수인계받은 배치를 직접 실행해보니 **14만 건 처리에 2시간이 소요**되어, 수동 동기화 시 사용자가 결과를 볼 때까지 2시간을 대기해야 하는 비효율이 있었습니다. 왜 이렇게 오래 걸리는지 코드를 뜯어보니 두 가지 병목이 있었습니다.',
        ...
        achievements: [
          {
            problem: '인수인계받은 배치를 직접 실행해보니 **14만 건 처리에 2시간 소요**. 수동 동기화 시 사용자가 결과를 볼 때까지 2시간 대기해야 하는 비효율',
