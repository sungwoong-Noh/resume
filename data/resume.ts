    ...
    projects: [
      {
        ...
        period: '2026.02 ~ 진행 중',
        achievements: [
          '고객사마다 별도 모니터링 체계가 없어 장애 발생 후에야 인지하는 구조적 문제 해소를 위해 기관별 Prometheus + Grafana 모니터링 서버 구축. CMP 개발팀과 보고서 자동 생성 기능 연계해 월간 리포트 수작업 제거',
          'KT Cloud 기반 멀티테넌트 인프라 모니터링 환경 구축 · 고객사별 자원 사용 가시성 확보',
          'Prometheus 기본 30일 보존 한계로 장기 추세 분석·장애 회고가 불가한 문제 인식. Thanos·Cortex는 확장성이 뛰어나나 S3 연동 설정 승인 및 러닝커브 필요 → 단일 바이너리·PromQL 호환·즉시 적용 가능한 VictoriaMetrics 선택. 보존 기간 30일 → 1년 확장 (Grafana 쿼리 범위로 직접 검증)',
          '다수 고객사 서버에 Exporter를 수동 SSH로 설치 시 누락·버전 불일치 위험 존재. 쉘 스크립트 대비 Ansible이 멱등성 보장·호스트 그룹 관리에 유리해 선택. Exporter 일괄 설치 및 자원 조회 자동화로 반복 운영 작업 제거',
    ...
    projects: [
      {
        ...
        achievements: [
          '3개 시스템이 별도 담당자 없이 분산 운영되어 이슈 발생 시 책임 소재 불명확·대응 지연 반복. RPMS·통합포털·ITSM 전 흐름을 단독으로 인수해 분류 체계·에스컬레이션 기준·배포 절차를 표준화',
          '130만 건 Tibero DB 핵심 조회 p95 17초로 현업 업무 지연이 반복 발생. DBA와 협업해 EXPLAIN으로 인덱스 미사용 원인 분석 → 힌트 강제 설정 시도했으나 미동작 → WHERE 절 타입 불일치(문자열 파싱)가 인덱스를 무력화하는 근본 원인 확인 → 타입 정합성 수정 후 인덱스 적용. 쿼리 실행 시간 직접 측정으로 개선 전후 비교, p95 17초 → 3초 (82% 개선)',
          '동일 오류가 반복 접수돼도 즉각 패치 후 종결하는 관행으로 근본 원인이 누적되는 문제 인식. 월 30건+ 운영 이슈를 유형별 분류·재발 빈도로 우선순위화해 고빈도 원인부터 순차 제거',
          '반복 접수되는 ITSM 오류 유형을 분석해 핵심 원인 위주로 기능 개선 적용. 인시던트 12건 → 5건/주 (주간 접수 건수 직접 집계로 추이 확인)',
        ],
      },
      {
        ...
        achievements: [
          ...
          '신규 CSP 플랫폼 초기 개발 단계에서 컨벤션·공통 모듈·에러 처리 기준이 없어 팀원마다 구현 방식이 달랐고 통합 시 충돌 빈번. 풀스택 리드로서 코딩 컨벤션·공통 모듈·에러 표준화를 초기에 정의해 일관성 확보',
          '클라우드 상품 신청 시 공인IP·방화벽·Cisco 스위치·OpenStack 등 다단계 장비 설정이 순차 실행되어 중간 실패 시 부분 설정이 잔존하는 문제 발생. 장비 API 트랜잭션과 Spring 애플리케이션 트랜잭션이 서로 다른 도메인이라 2PC 적용 불가 → 각 단계별 보상 로직을 직접 구현하는 Saga 패턴 선택. 장비 설정충돌 오류 0건 달성',
          '기존 티켓 기반 CSP 방식은 사용자가 자원 신청 시마다 운영자가 수동으로 처리해야 했고, 이는 CSP의 핵심 가치인 사용자 자율 자원 관리를 불가능하게 하는 구조적 한계. AWS·Azure처럼 API를 통해 사용자가 직접 자원을 제어하는 Self-Service 구조로 전환 설계·구현. 수동 운영 워크로드 90% 절감 (월 50건 → 5건)',
          '\'어차피 손으로 눌러보면 된다\'는 팀 문화 속에서 핵심 기능 점진 적용을 제안·합의 → JUnit5 단위테스트 문화 정착 · 설정오류 0건 · 개발 속도 30% 향상',
          '유사 기능이 여러 모듈에 중복 구현되어 수정 시 전 모듈을 개별 패치해야 하는 유지보수 비용 발생. 중복 로직을 공통 모듈로 통합하고 의존 관계를 단방향으로 정리. 재사용성 60% 향상',
        ],
      },
      {
        ...
        achievements: [
          ...
          '배치 동기화에서 N+1 쿼리로 14만 건 처리에 2시간 소요, 야간 배치 완료 전 업무 시작으로 데이터 정합성 이슈 발생. 조회 측 N+1은 Fetch Join으로 해결. INSERT 측은 JPA IDENTITY 전략이 영속성 컨텍스트 PK 확보를 위해 단건 INSERT를 강제하는 구조적 한계를 확인 → JDBC Template Bulk Insert로 전환. 로컬 테스트 코드로 실행 시간 측정, 배치 처리 10초로 단축 (99% 개선)',
          '요구사항 불명확으로 인한 재작업과 수동 배포 대기 시간이 업무 리드타임을 늘리는 주요 원인. 요구사항 정제 프로세스 도입과 Jenkins 배포 자동화를 병행 적용. 업무 리드타임 5일 → 3일 단축',
          'Native 쿼리 남용으로 조건 변경 시마다 SQL 직접 수정이 필요해 유지보수 비용이 급증하는 문제 인식. JPA 연동 타입 안전 쿼리 빌더인 QueryDSL을 검토, 조회 복잡도가 높은 자산 현황 모듈을 선택해 파일럿 적용 후 팀의 초기 회의론을 실측 데이터로 설득. 전사 도입 주도 → Native 쿼리 70%+ 제거 · 유지보수 비용 50% 절감',
    ...
    projects: [
      {
        ...
        achievements: [
          ...
          '동시 신청 시 이중 예약 가능성 존재. Redis 분산락은 별도 인프라 구성 필요, DB 비관적 락은 소규모 행사 트래픽에 과도한 설계 → 별도 인프라 없이 임시 예약 상태를 DB로 관리하는 Soft Reservation 선택. 이중 신청 0건',
      ...
      },
      {
        ...
        achievements: [
          ...
          '헤더에서 매 페이지 이동 시마다 전체 SMS 카운트 집계 쿼리가 자동 실행되어 페이지 로딩 지연 반복. 상시 표시가 불필요한 집계 쿼리임을 파악 → 버튼 클릭 시에만 조회하는 방식으로 변경. 쿼리 실행 시간 직접 측정, p95 5초 → 0.9초 · CPU 사용률 65% → 40% 개선',
      ...
      },
      {
        ...
        achievements: [
          ...
          'JSP 페이지마다 헤더·푸터·네비게이션을 반복 복붙하는 중복 레이아웃 문제 인식. JSP include 방식은 중복 제거 효과가 제한적이며 구조 변경 시 전 페이지 수정 필요 → 레이아웃 템플릿화로 전체 페이지를 재사용 구조로 만들 수 있는 Tiles 프레임워크 도입 제안 및 적용. 중복 코드 65% → 10% · 페이지 개발 리드타임 2일 → 0.5일',
          '수동 배포로 인한 휴먼에러와 7일 배포 주기가 긴급 패치 대응을 가로막는 문제 인식. SI 폐쇄망·온프레미스 환경 특성상 관리형 SaaS CI(GitHub Actions 등) 사용 불가 → 자체 구축 가능한 Jenkins 선택해 직접 파이프라인 구성. 배포 빈도 7일 → 1일',
