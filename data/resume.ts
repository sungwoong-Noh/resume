...
export const profile = {
   ...
   */
  intro: `**문제의 원인을 구조 레벨까지 파고들어 해결하는 백엔드 개발자 노성웅입니다.**
...
export const experiences: Experience[] = [
    ...
    projects: [
      {
        ...
        resumeContent: [
          {
            ...
            body: [
              '국정자원 공무원들이 자산을 조회하는 기능인데, 이상한 증상이 있었습니다. **ID로 검색하면 순식간에 나오는데, 관리번호나 상태 같은 조건을 하나 추가하면 응답이 급격히 느려졌습니다.** 130만 건 규모에서 **p95가 17초**까지 치솟아, 실무자들이 조건을 걸어 자산을 찾을 때마다 매번 십수 초씩 화면이 멈췄습니다.',
              'ID 검색만 빠르다는 건 인덱스 자체는 정상인데 조건을 추가할 때만 안 탄다는 뜻이라, **실행계획을 떠서** 확인했습니다. 원인은 **WHERE절 컬럼 타입과 파라미터 타입 불일치**였습니다. 타입이 다르니 DB가 컬럼 쪽에 형변환을 걸었고, 그 순간 **인덱스가 무력화돼 풀스캔**으로 떨어지고 있었습니다.',
              '타입 정합성을 맞춰 인덱스를 다시 타도록 수정했습니다.',
            ],
        ...
        achievements: [
          {
            problem: 'ITSM 자산 조회에서 **ID 검색은 빠른데 조건을 추가하면 급격히 느려지는** 문제. **130만 건 규모에서 p95 17초**로 현업 공무원 업무 지연 발생.',
            analyze: '실행계획을 분석해 **WHERE절 컬럼 타입과 파라미터 타입 불일치로 인덱스가 무력화**되고 있다는 원인을 파악, 타입 정합성 수정.',
            result: '자산 조회 응답속도 **p95 17초 → 3초** (82% 개선)',
          },
        ],
      },
      {
        ...
        resumeContent: [
          {
            subtitle: '멀티벤더 프로비저닝 오케스트레이션 구조 설계',
            body: [
              '백엔드 5인 팀에서 **로드밸런서(L4)·포트포워딩 상품 개발**을 담당했습니다. 고객 하나에게 네트워크 환경을 만들어주려면 방화벽 계정 생성부터 VS·인터페이스·라우트 설정, ACI 초기화, OpenStack 네트워크 생성까지 **서로 다른 벤더의 장비 API를 정해진 순서로** 호출해야 했습니다. 문제는 이 장비들이 전부 외부 시스템이라 **DB 트랜잭션은 물론 Spring @Transactional로도 묶을 수 없다**는 점이었습니다.',
              '그래서 중간 한 단계라도 실패하면 앞 단계에서 만든 자원이 그대로 남았습니다. 예를 들어 포트포워딩 하나를 설정하려면 출발지·도착지·포트 객체를 순서대로 생성하는데, **출발지만 만들어진 채 실패하면 다음 시도에서 "이미 존재한다"며 재시도조차 막혔습니다.** 게다가 이 흐름 전체가 **150~250줄짜리 한 서비스 메서드**에 몰려 있었고, 롤백도 단계마다 try-catch로 흩어져 중복됐으며 실패 지점에 따라 처리 방식이 제각각이었습니다.',
              '먼저 **벤더 호출 순서를 조율하는 Integration 계층을 신설**해, 순수 DB 로직(도메인 서비스)과 벤더 어댑터(external 서비스)를 분리하고 엔티티 조립은 별도 Builder로 떼어내 오케스트레이션과 데이터 구성 책임을 나눴습니다.',
              '그리고 프로비저닝 7단계(계정·VS·인터페이스·라우트·ACI·OpenStack·DB)마다 **실패 지점별로 롤백 범위가 달라야 한다**고 판단해, 단계별로 세분화한 **11개 예외 타입 체계**를 제안·구현했습니다. 공통 예외는 상속으로 재사용하고 하위 예외마다 롤백에 필요한 컨텍스트만 얹었습니다. 이 뼈대 위에서 **각 도메인 담당자는 자기 단계의 롤백만 정의하면 되도록** 만들어, 공통 구조와 도메인 지식을 분리했습니다.',
            ],
          ...
          },
          {
            ...
            body: [
              '네트워크 상품은 고객(테넌트)마다 독립된 환경을 보장해야 했습니다. 방화벽·LB 등 각 벤더 장비가 실시간 상태 조회 API를 자체 제공하고 있어, **DB가 장비의 전체 상태를 복제해 들고 있을 필요는 없었습니다.** 테넌트-자원 매핑과 오케스트레이션에 필요한 최소한의 서비스 데이터만 저장하면 됐기 때문에, 테넌트별 스키마를 분리할 만큼 데이터 볼륨·복잡도가 크지 않다고 판단해 **단일 스키마 위에서 FK 그래프로 논리 격리하는 구조**를 택했습니다. 다만 **상품 도메인마다 격리 특성이 달라**, 방화벽과 LB의 모델링은 다르게 가져갔습니다.',
              '방화벽은 **Zone(테넌트)을 루트로 하는 엄격한 FK 그래프**로 모델링했습니다. **Zone PK 컬럼을 테넌트 경계의 루트 엔티티에만 두고**, 하위 엔티티(인터페이스·공인IP·포트포워딩)는 상위로 조인해 올라가야 테넌트를 식별하도록 정규화했습니다.',
              '반면 로드밸런서는 **비정규화 구조**로 갔습니다. DB 레벨 테넌트 식별자와 장비(Citrix) 레벨 식별자를 한 엔티티에 나란히 저장해, DB 조회와 장비 API 호출에 각각 맞는 키를 즉시 쓸 수 있게 했습니다. 여기에 **DB는 FK 논리 격리, 물리 장비는 테넌트별 파티션**(TrusGuard의 VS, Citrix의 Partition)으로 **2단 격리**를 구성했습니다.',
            ],
            result: '도메인마다 격리 요구가 다른 멀티테넌트 상품을 단일 스키마 위에서 일관되게 관리 · DB·장비 양쪽 테넌트 키를 함께 다뤄 조회·프로비저닝 경로 단순화',
          },
          {
            subtitle: 'Self-Service API 전환으로 반복 운영 요청 대폭 절감',
            body: [
              '고객은 자기 자원을 직접 만들 수 없고, 신청하면 운영자가 대신 생성해주는 **매니지드 방식**이었습니다. 방화벽 정책 하나, 포트포워딩 하나를 바꾸려 해도 요청을 넣고 담당자가 처리해줄 때까지 기다려야 했고, 이런 반복 요청이 **체감상 월 50건 안팎**으로 운영자 손이 계속 묶였습니다. 고객 입장에선 자기 자원을 직접 제어할 수 없는 **반쪽짜리 CSP**였습니다.',
          ...
          },
          {
            ...
            body: [
              '신규 플랫폼이라 시작할 때 컨벤션도, 공통 모듈도, 에러 처리 기준도 없었습니다. 팀원마다 응답 형식이나 예외 처리를 제각각 구현하다 보니 **같은 로직이 여기저기 중복**됐고, 나중에 남의 코드를 유지보수하려면 "이건 어떤 방식으로 짰지"부터 파악해야 하는 구조였습니다.',
              '선임 개발자와 함께 **코딩 컨벤션·공통 모듈·에러 처리 표준을 수립**해 구조를 정비하고, **JUnit5 기반 테스트를 핵심 기능부터 점진적으로 도입**했습니다. 특히 **장비 API를 실제 호출하는 통합 테스트**로 스펙 불일치나 IP 충돌을 배포 전에 잡도록 했습니다.',
            ],
            result: '**팀 전체가 동일한 개발 기준으로 작업하는 구조 정착** · 장비 API 스펙 불일치·IP 충돌을 테스트로 사전 검증',
          },
        ],
        achievements: [
          {
          ...
          },
          {
            ...
            analyze: '선임 개발자와 함께 **코딩 컨벤션·공통 모듈·에러 처리 표준 수립** + **JUnit5 기반 테스트를 핵심 기능부터 도입**(장비 API를 실제 호출하는 통합 테스트 포함)해 수동 검증 문화 전환',
            result: '**팀 전체가 동일 개발 기준으로 작업하는 구조 정착** · 장비 API 스펙 불일치·IP 충돌을 테스트로 사전 검증',
          },
        ],
      },
      {
        ...
        serviceOverview: '국가정보자원관리원의 IT 자산을 계획·관리하는 시스템으로, 공공 예산심사와 감사 대응이 걸려 있어 자산 데이터의 정확성과 변경 이력 추적이 핵심입니다.',
        resumeContent: [
          {
          ...
          {
            subtitle: '예산심사 감사 요구사항을 반영한 diff 기반 변경 이력 설계',
            body: [
            ...
            result: '감사 대응에 필요한 변경 이력 추적을 만족시키면서, 심사자에게는 **실제 변경분만 노출되는 신뢰도 있는 이력** 제공',
          },
          {
            subtitle: '사업유형별 검증 규칙 분리(Bean Validation Group)로 설계 오류 예방',
            body: [
              '부분설계 등록 시 **모든 자원 신청에 동일한 필수값 검증**이 적용되는 구조였습니다. 하지만 실제 심사 규정은 **사업유형(신규·증설·추가·노후·폐기)마다 요구 항목이 달랐습니다.** 예를 들어 노후로 폐기 예정인 자산은 CPU·메모리 같은 상세 스펙이 필요 없는데도 신규 도입과 똑같은 필수값을 요구하니, 실무자가 **뭘 채워야 할지 헷갈려 잘못된 설계로 이어질** 수 있었습니다.',
              '**Bean Validation의 Validation Group을 사업유형별로 분리**해, 사업유형 코드에 따라 **런타임에 검증 규칙 자체가 다르게 적용**되도록 설계했습니다. 폐기 대상은 최소 식별 정보만 검증하고, 신규 도입은 스펙 필드까지 엄격히 검증하는 식입니다.',
            ],
            result: '사업유형별로 다른 심사 규정을 **코드 구조로 반영**해, 실무자 입력 실수로 이어질 수 있는 설계 오류를 예방',
          },
          {
        ...
        achievements: [
          ...
          {
            problem: '공공 예산심사 시스템이라 **감사 대응상 "누가 언제 무엇을 변경했는지" 이력 추적**이 필요. 단순 이력 적재 시 **값 변경 없는 저장에도 이력이 쌓여** 심사자가 볼 실제 변경분이 노이즈에 묻힘',
            analyze: '서버·스토리지·네트워크 등 **7종 자원을 요청 값과 기존 데이터로 필드 단위(자원당 20여 개) 비교해 실제 변경 시에만 이력 적재**. 폼 입력값(문자열)·엔티티 값(숫자·boolean) 타입 차이 오탐도 처리',
            result: '감사 이력 요구 충족 + **실제 변경분만 노출되는 신뢰도 있는 이력** 제공',
          },
          {
            problem: '부분설계 등록 시 모든 자원에 동일 필수값 검증이 걸리는데, **실제 심사 규정은 사업유형(신규·증설·폐기 등)마다 요구 항목이 달라** 폐기 대상에도 스펙 입력을 요구하는 등 실무자 혼란·설계 오류 유발',
            analyze: '**Bean Validation의 Validation Group을 사업유형별로 분리**, 사업유형 코드에 따라 **런타임에 검증 규칙이 다르게 적용**되도록 설계 (폐기는 최소 식별 정보만, 신규 도입은 스펙까지 엄격 검증)',
            result: '사업유형별 심사 규정을 **코드 구조로 반영**해 입력 실수·설계 오류 예방',
          },
          {
  ...
  },
  {
    ...
    highlights: [
      'Soft Reservation 설계로 동시 신청 이중 예약 방지 · 이중 신청 0건',
    ...
    resumeParagraphs: [
      ...
      '행사 신청 플랫폼에서는 **동시 신청이 몰리면 이중 예약이 발생하는 동시성 문제**를 설계로 풀었습니다. Redis 분산락과 DB 비관적 락을 검토한 뒤 **소규모 트래픽에 과한 설계라 판단해 배제**하고, 별도 인프라 없이 DB 임시 예약 상태로 관리하는 **Soft Reservation 방식으로 이중 신청 0건**을 달성했습니다.',
    ],
