    ...
    projects: [
      {
        ...
        resumeContent: [
          ...
          },
          {
            ...
            body: [
              '고객이 네트워크 환경을 하나 만들려면 방화벽 계정 생성부터 VS·인터페이스·라우트 설정, ACI 초기화, OpenStack 네트워크 생성까지 **서로 다른 벤더의 장비 API를 정해진 순서로** 호출해야 했습니다. 이 장비들은 전부 외부 시스템이라 **DB 트랜잭션(과 이를 자동화해주는 Spring @Transactional)이 커버하는 범위 밖**에 있어서, 중간 한 단계라도 실패하면 앞서 만든 자원이 그대로 남는 **자원 누수**가 발생했습니다. 예를 들어 방화벽 계정은 만들어졌는데 VS 생성 단계에서 실패하면, 다음 시도에서 계정이 **"이미 존재한다"며 재시도조차 막혔습니다.**',
