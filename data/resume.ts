      ...
      },
      {
        ...
        resumeContent: [
          ...
          },
          {
            ...
            body: [
              '네트워크 상품은 고객(테넌트)마다 독립된 환경을 보장해야 했습니다. 방화벽·LB 등 각 벤더 장비가 실시간 상태 조회 API를 자체 제공하고 있어, **DB가 장비의 전체 상태를 복제해 들고 있을 필요는 없었습니다.** 테넌트-자원 매핑과 오케스트레이션에 필요한 최소한의 서비스 데이터만 저장하면 됐기 때문에, 고객별로 데이터베이스를 나눌 만큼 볼륨·복잡도가 크지 않다고 판단해 하나의 데이터베이스 안에서 고객별로 논리적으로 구분하는 구조를 택했습니다.',
            ...
            result: '상품마다 다른 격리 요구사항을 하나의 데이터베이스 구조로 일관되게 관리 · 데이터베이스와 실제 장비 양쪽의 식별자를 함께 다뤄 처리 경로 단순화',
          },
          {
