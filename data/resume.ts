...
export const profile = {
   ...
   */
  intro: `JPA 내부 동작을 이해하고 실행계획을 분석해 쿼리 구조를 재설계했습니다. N+1 문제로 2시간이 걸리던 동기화를 10초로 단축하고, 130만 건 DB p95를 17초에서 3초로 개선한 것처럼 지시받은 기능 구현보다 문제를 먼저 찾아 근본 원인을 해결하는 방식으로 일해왔습니다.
    ...
    projects: [
      {
        ...
        achievements: [
          ...
          'Prometheus 기본 30일 보존 한계로 장기 추세 분석·장애 회고가 불가한 문제 인식. Thanos·Cortex는 확장성이 뛰어나나 S3 연동 설정 승인 및 러닝커브 필요 → 단일 바이너리·PromQL 호환·즉시 적용 가능한 VictoriaMetrics 선택. 메트릭 보존 기간 30일 → 1년 확장',
    ...
    projects: [
      {
        ...
        achievements: [
          ...
          '130만 건 Tibero DB 핵심 조회 p95 17초로 현업 업무 지연이 반복 발생. DBA와 협업해 EXPLAIN으로 인덱스 미사용 원인 분석 → 힌트로 강제 설정 시도했으나 미동작 → WHERE 절 타입 불일치(문자열 파싱)가 인덱스를 무력화하는 근본 원인 확인 → 타입 정합성 수정 후 인덱스 적용. p95 17초 → 3초 (82% 개선)',
      ...
      },
      {
        ...
        achievements: [
          ...
          '배치 동기화에서 N+1 쿼리로 14만 건 처리에 2시간 소요, 야간 배치 완료 전 업무 시작으로 데이터 정합성 이슈 발생. 조회 측 N+1은 Fetch Join으로 해결. INSERT 측은 JPA IDENTITY 전략이 영속성 컨텍스트 PK 확보를 위해 단건 INSERT를 강제하는 구조적 한계를 확인 → JDBC Template Bulk Insert로 전환. 배치 처리 10초로 단축 (99% 개선)',
