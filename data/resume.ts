        ...
        resumeContent: [
          {
            ...
            body: [
              ...
              '그리고 프로비저닝 7단계(계정·VS·인터페이스·라우트·ACI·OpenStack·DB)마다 **실패 지점별로 롤백 범위가 달라야 한다**고 판단해, 단계별로 세분화한 **11개 예외 타입 체계**를 제안·구현했습니다. 공통 예외는 상속으로 재사용하고 하위 예외마다 롤백에 필요한 컨텍스트만 얹었습니다. 이 뼈대 위에서 **각 도메인 담당자는 자기 단계의 롤백만 정의하면 되도록**(계정 단계는 계정만, OpenStack 단계는 역순 삭제) 만들어, 공통 구조와 도메인 지식을 분리했습니다.',
            ],
          ...
          },
          {
            ...
            body: [
              '네트워크 상품은 고객(테넌트)마다 독립된 환경을 보장해야 하는데, **상품 도메인마다 격리 특성이 달랐습니다.** 테넌트별로 DB를 나누는 스키마 분리는 상품·고객이 늘수록 운영 부담이 커져서, **단일 스키마 위에서 FK 그래프로 논리 격리**하되 도메인 특성에 맞게 구조를 다르게 설계하기로 했습니다.',
