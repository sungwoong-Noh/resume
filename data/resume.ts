        ...
        resumeContent: [
          {
            ...
            body: [
              '당시 고객은 자기 자원을 직접 만들 수 없고, 신청하면 운영자가 대신 생성해주는 **매니지드 방식**이었습니다. 로드밸런서에 VM 하나를 연결하려 해도 요청을 넣고 담당자가 처리해줄 때까지 기다려야 했고, 이런 반복 요청이 **체감상 월 50건 안팎**으로 운영자 손이 계속 묶였습니다. 고객 입장에선 자기 자원을 직접 제어할 수 없는 **제한적인 CSP 구조**였습니다.',
              '**네트워크 장비가 이미 멀티테넌트 기능을 제공한다는 점에 착안**해, 이 기능을 외부에 안전하게 노출하는 별도 API 서버를 신규 개발했습니다. 고객이 자기 테넌트 범위 안에서 직접 자원을 신청하면 즉시 처리되도록 흐름을 바꿔, 이전엔 제한적이었던 CSP 환경을 온전한 셀프서비스 형태로 제공할 수 있었습니다.',
            ],
            result: [
              '매니지드 방식에서 셀프 서비스로 처리되는 구조로 전환',
        ...
        achievements: [
          {
            ...
            result: '매니지드 방식에서 셀프 서비스로 처리되는 구조로 전환, **체감상 월 50건 안팎이던 반복 요청이 한 자릿수 수준으로 감소**',
          },
          {
