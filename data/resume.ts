...
export const experiences: Experience[] = [
  {
    ...
    highlights: [
      ...
      '멀티벤더 프로비저닝을 Saga 패턴(11개 예외 타입 기반 차등 롤백)으로 설계 · 실패 시 자원 고아 방지',
    ...
    projects: [
      {
        ...
        resumeContent: [
          {
            ...
            body: [
              '백엔드 5인 팀 중 **로드밸런서(L4)·포트포워딩 상품**은 전담하여 개발했습니다. 당시 고객은 자기 자원을 직접 만들 수 없고, 신청하면 운영자가 대신 생성해주는 **매니지드 방식**이었습니다. 방화벽 정책 하나, 포트포워딩 하나를 바꾸려 해도 요청을 넣고 담당자가 처리해줄 때까지 기다려야 했고, 이런 반복 요청이 **체감상 월 50건 안팎**으로 운영자 손이 계속 묶였습니다. 고객 입장에선 자기 자원을 직접 제어할 수 없는 **제한적인 CSP 구조**였습니다.',
          ...
          },
          {
            ...
            body: [
              ...
              '순수 도메인 로직과 벤더 어댑터(external)를 분리하는 **Integration 계층을 신설**해 프로비저닝 파이프라인을 구조화하고, 엔티티 조립은 별도 Builder로 떼어냈습니다. 이렇게 흐름을 한 곳에 모으고 나니 프로비저닝 7단계(계정·VS·인터페이스·라우트·ACI·OpenStack·DB)마다 **실패 지점을 명확히 구분**할 수 있었습니다.',
            ...
            result: '장비가 응답한 실패에 한해 실패 지점과 무관하게 **자원이 고아 상태로 남지 않도록 구조적으로 보장** · 벤더 추가 시 예외 타입·핸들러만 추가하는 확장 구조 확립',
          },
          {
            subtitle: '로드밸런서 공유 자원의 참조 관리 설계',
            body: [
              'Citrix 장비에서 백엔드 서버는 **{IP}_{PORT}로 식별되는 하나의 Service 객체**인데, 여러 개의 로드밸런서(가상서버)가 같은 백엔드를 공유해서 바인딩할 수 있다는 게 이 상품의 특성이었습니다. 문제는 **장비 자체엔 이 Service를 몇 개의 가상서버가 참조 중인지 세는 개념이 없다**는 것이었습니다. VM을 연결할 때 이미 등록된 Service가 있으면 중복 생성하면 안 됐고, 연결을 해제할 때는 다른 가상서버가 아직 그 Service를 쓰고 있는지 모른 채 삭제하면 엉뚱한 서비스의 백엔드가 끊길 위험이 있었습니다.',
            ...
            result: '상태를 갖지 않는 벤더 API 위에서 **여러 가상서버가 백엔드를 안전하게 공유할 수 있는 구조 확보** · 중복 생성과 사용 중인 자원의 조기 삭제를 구조적으로 방지',
          },
          {
        ...
        achievements: [
          ...
          },
          {
            ...
            result: '장비 응답 실패에 한해 **자원 고아 상태 구조적 방지** · 벤더 추가 시 예외 타입·핸들러만 추가하는 확장 구조',
          },
          {
            problem: 'Citrix 장비에서 백엔드는 **{IP}_{PORT}로 식별되는 하나의 Service 객체**인데 여러 로드밸런서(가상서버)가 같은 Service를 공유 바인딩 가능. **장비 자체엔 참조 카운트 개념이 없어** 중복 생성·사용 중인 자원의 조기 삭제 위험',
            ...
            result: '상태를 갖지 않는 벤더 API 위에서 **여러 가상서버의 백엔드 공유를 안전하게 구조적으로 보장** · 중복 생성·조기 삭제 방지',
          },
          {
