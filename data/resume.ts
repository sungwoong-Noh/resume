    ...
    projects: [
      {
        ...
        resumeContent: [
          ...
          {
            subtitle: '벤더 장비 스펙 불일치를 통합 테스트로 사전 검증',
            body: [
              'PPP Cloud의 네트워크 상품은 방화벽·로드밸런서 등 **테넌트의 최전방 네트워크 장비를 전부 다루는 구조**라, 이 설정 하나가 실패하면 테넌트 네트워크 자체가 구성되지 않아 방금 만든 VM에도 도달할 수 없었습니다. 그런데 **벤더 장비마다 전달받은 스펙 문서와 실제 동작이 달랐습니다.** 안랩 방화벽은 문서의 API 설정 필드명에 오타가 있었고, Citrix는 쿼리 파라미터를 독자적인 방식으로 인코딩해야 했습니다.',
              '사내엔 테스트 코드를 작성하는 문화 자체가 없었지만, 연동 기능을 개발할 때마다 이런 문제로 고치고 다시 배포하는 과정이 반복되자 사전 검증이 필요하다고 판단했습니다. **Mock 없이 실제 스테이징 장비(TrusGuard·Citrix·ACI 등)를 직접 호출하는 통합 테스트**로 설계해, 각 테스트가 장비 위에 실제 상태를 만들고 정리하는 것까지 스스로 책임지도록 만들었습니다.',
              '이 테스트의 가치는 비즈니스 로직 검증보다, **우리 쪽 요청 포맷이 실제 장비 API 스펙과 어긋나지 않는지**를 배포 전에 확인하는 데 있었습니다. 최전방 네트워크 설정 실패가 테넌트 전체 연결 장애로 번지는 상황을 사전에 막을 수 있었습니다.',
            ],
            result: [
              'Mock 없이 실제 벤더 장비(TrusGuard·Citrix·ACI 등)를 직접 호출하는 통합 테스트로, **요청 포맷이 실제 장비 스펙과 어긋나지 않는지 배포 전 검증**',
              '최전방 네트워크 설정 실패가 **테넌트 전체 연결 장애로 번지는 상황을 사전 차단**',
            ],
          },
        ],
        achievements: [
          ...
          },
          {
            ...
            analyze: '테스트 코드 문화가 없던 조직에서, **Mock 없이 실제 스테이징 장비를 직접 호출하는 통합 테스트**를 도입 — 비즈니스 로직보다 **요청 포맷이 실제 장비 스펙과 어긋나지 않는지**를 배포 전 검증하는 데 초점',
