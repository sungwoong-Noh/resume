    ...
    projects: [
      {
        ...
        resumeContent: [
          ...
          },
          {
            ...
            body: [
              '고객이 네트워크 환경을 하나 만들려면 방화벽 계정 생성부터 VS·인터페이스·라우트 설정, ACI 초기화, OpenStack 네트워크 생성까지 **서로 다른 벤더의 장비 API를 정해진 순서로** 호출해야 했습니다. 이 장비들은 전부 외부 시스템이라 **DB 트랜잭션은 물론 Spring @Transactional로도 묶을 수 없었습니다.**',
          ...
          },
          {
            ...
            body: [
              '프로비저닝 7단계(계정·VS·인터페이스·라우트·ACI·OpenStack·DB)마다 **실패 지점별로 롤백 범위가 달라야 한다**고 판단해, 단계별로 세분화한 **11개 예외 타입 체계**를 제안·구현했습니다.',
