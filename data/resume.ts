        ...
        resumeContent: [
          {
            ...
            body: [
              '백엔드 5인 팀 중 **로드밸런서(L4)·포트포워딩 상품**은 담당을 맡아 혼자 개발했습니다. 당시 고객은 자기 자원을 직접 만들 수 없고, 신청하면 운영자가 대신 생성해주는 **매니지드 방식**이었습니다. 방화벽 정책 하나, 포트포워딩 하나를 바꾸려 해도 요청을 넣고 담당자가 처리해줄 때까지 기다려야 했고, 이런 반복 요청이 **체감상 월 50건 안팎**으로 운영자 손이 계속 묶였습니다. 고객 입장에선 자기 자원을 직접 제어할 수 없는 **반쪽짜리 CSP**였습니다.',
      ...
      },
      {
        ...
        resumeContent: [
          ...
          },
          {
            ...
            body: [
              'RPMS는 공공기관 예산심사 시스템이라 **"누가 언제 무엇을 변경했는지" 추적 가능해야 하는 감사 요구사항**이 있었습니다. 그런데 단순히 "수정 요청이 오면 이력을 남긴다"로 구현하면, 실제 값 변경이 없는 저장 요청에도 **불필요한 이력이 과다 적재되어 감사 추적성(Traceability)을 오히려 저해하는** 문제가 생길 수 있다고 봤습니다.',
