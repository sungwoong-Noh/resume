    ...
    projects: [
      {
        ...
        resumeContent: [
          ...
          },
          {
            ...
            body: [
              '고객이 네트워크 환경을 하나 만들려면 방화벽 계정 생성부터 VS·인터페이스·라우트 설정, ACI 초기화, OpenStack 네트워크 생성까지 **서로 다른 벤더의 장비 API를 정해진 순서로** 호출해야 했습니다. 이 장비들은 전부 외부 시스템이라 **DB 트랜잭션(과 이를 자동화해주는 Spring @Transactional)이 커버하는 범위 밖**에 있어서, 중간 한 단계라도 실패하면 앞서 만든 자원이 그대로 남는 **자원 누수가 발생할 수 있는 구조**였습니다. 예를 들어 방화벽 계정은 만들어졌는데 VS 생성 단계에서 실패하면, 다음 시도에서 계정이 **"이미 존재한다"며 재시도조차 막혔습니다.**',
              '순수 도메인 로직과 벤더 어댑터를 분리하는 **Integration 계층을 신설**해 프로비저닝 파이프라인을 구조화했습니다. 프로비저닝 7단계(계정·VS·인터페이스·라우트·ACI·OpenStack·DB)마다 **실패 지점을 명확히 구분**하고, 단계별로 롤백 범위가 다른 **11개의 계층형 예외 타입**을 설계해 **보상 트랜잭션 기반의 오케스트레이션(Saga 패턴)**을 구현했습니다. 공통 예외는 상속으로 재사용하고, 하위 예외마다 롤백 컨텍스트만 얹어 **각 도메인 담당자가 자기 단계의 롤백만 채우면 되도록** 만들었습니다. **그 결과 새 벤더가 추가되거나 도메인 로직이 바뀌어도 기존 흐름을 건드리지 않고 확장할 수 있었습니다.**',
            ],
            result: [
          ...
          },
          {
            ...
            body: [
              'Citrix 장비에서 백엔드 서버는 **{IP}_{PORT}로 식별되는 하나의 Service 객체**인데, 여러 개의 로드밸런서가 같은 백엔드를 공유해서 바인딩할 수 있다는 게 이 상품의 특성이었습니다. 문제는 **여러 가상서버가 이 Service를 안전하게 공유하려면, 애플리케이션이 참조 상태를 직접 추적하고 있어야 했다**는 것이었습니다.',
              'VM을 연결할 때 이미 등록된 Service가 있으면 중복 생성하면 안 됐습니다. 반대로 연결을 해제할 때는 다른 가상서버가 아직 그 Service를 쓰고 있는지 모른 채 삭제하면, 엉뚱한 서비스의 백엔드가 끊길 위험이 있었습니다.',
