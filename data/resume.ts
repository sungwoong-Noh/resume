      ...
      },
      {
        ...
        resumeContent: [
          {
            ...
            body: [
              '백엔드 5인 팀에서 **로드밸런서(L4)·포트포워딩 상품 개발을 담당**했습니다. 네트워크 환경 하나를 만들려면 방화벽·VS·인터페이스·라우트·ACI·OpenStack 등 **서로 다른 벤더의 API를 순서대로 호출**해야 했는데, 이들은 외부 시스템이라 **DB 트랜잭션은 물론 Spring @Transactional로도 묶을 수 없었습니다**. 중간 단계가 실패하면 이미 만들어진 자원이 그대로 남아, 재시도조차 막히는 상황이 발생했습니다.',
              '벤더 호출을 조율하는 **Integration 계층을 분리**하고, 프로비저닝 7단계마다 **실패 지점별로 롤백 범위가 다른 11개 예외 타입 체계**를 제안·구현했습니다. 도메인별 구체 롤백 로직은 각 도메인 담당자가 이 구조 위에서 채우도록 했습니다.',
            ],
          ...
          },
          {
            ...
            body: [
              '고객(테넌트)마다 독립된 네트워크 환경을 제공해야 했는데, **상품 도메인마다 격리 특성이 달라** 단일 방식으로 모델링할 수 없었습니다. 스키마 분리는 상품·고객이 늘수록 운영 부담이 커, **단일 스키마 + FK 그래프로 논리 격리**하되 도메인 특성에 맞게 구조를 다르게 설계했습니다.',
              '방화벽은 Zone(테넌트)을 루트로 하는 **엄격한 FK 그래프**로, LB는 DB 식별자와 장비(Citrix) 식별자를 **함께 저장하는 비정규화 구조**로 설계했습니다. DB는 FK 논리 격리, 물리 장비는 테넌트별 파티션으로 **2단 격리**를 구성했습니다.',
            ],
          ...
          },
          {
            ...
            body: [
              '고객이 자원을 신청하면 운영자가 직접 내부 시스템에서 자원을 생성해야 하는 티켓 방식이었고, **반복 작업이 월 50건**에 달해 고객이 직접 자원을 제어할 수 없는 **반쪽짜리 CSP** 구조였습니다.',
          ...
          },
          {
            ...
            body: [
              '신규 플랫폼 초기 개발이라 컨벤션·공통 모듈·에러 처리 기준이 없어 **팀원마다 구현 방식이 달라 중복 코드가 쌓이고 유지보수가 어려운 구조**였습니다.',
              '선임 개발자와 함께 **코딩 컨벤션·공통 모듈·에러 처리 표준을 수립**하고, **JUnit5 단위테스트를 핵심 기능부터 점진적으로 도입**해 수동 검증 문화를 바꿨습니다.',
            ],
        ...
        achievements: [
          ...
          },
          {
            ...
            analyze: '선임 개발자와 함께 **코딩 컨벤션·공통 모듈·에러 처리 표준 수립** + **JUnit5 단위테스트를 핵심 기능부터 도입**해 수동 검증 문화 전환',
