        ...
        resumeContent: [
          {
            ...
            body: [
              ...
              '그리고 프로비저닝 7단계(계정·VS·인터페이스·라우트·ACI·OpenStack·DB)마다 **실패 지점별로 롤백 범위가 달라야 한다**고 판단해, 단계별로 세분화한 **11개 예외 타입 체계**를 제안·구현했습니다. 공통 예외는 상속으로 재사용하고 하위 예외마다 롤백에 필요한 컨텍스트만 얹었습니다. 도메인별 구체 롤백 로직(계정 단계는 계정만 삭제, OpenStack 단계는 역순 삭제)은 각 도메인을 가장 잘 아는 담당자가 이 구조 위에서 채우도록 했습니다.',
            ],
          ...
          },
          {
            ...
            body: [
              ...
              '방화벽은 **Zone(테넌트)을 루트로 하는 엄격한 FK 그래프**로 모델링했습니다. zone_idx 컬럼을 테넌트 경계의 루트 엔티티에만 두고, 하위 엔티티(인터페이스·공인IP·포트포워딩)는 상위로 조인해 올라가야 테넌트를 식별하도록 정규화했습니다.',
