    ...
    projects: [
      {
        ...
        resumeContent: [
          {
            ...
            body: [
              '백엔드 5인 팀 중 **로드밸런서(L4)·포트포워딩 상품**은 전담하여 개발했습니다. 당시 고객은 자기 자원을 직접 만들 수 없고, 신청하면 운영자가 대신 생성해주는 **매니지드 방식**이었습니다. 로드밸런서에 VM 하나를 연결하려 해도 요청을 넣고 담당자가 처리해줄 때까지 기다려야 했고, 이런 반복 요청이 **체감상 월 50건 안팎**으로 운영자 손이 계속 묶였습니다. 고객 입장에선 자기 자원을 직접 제어할 수 없는 **제한적인 CSP 구조**였습니다.',
          ...
          },
          {
            ...
            result: '실패 지점과 무관하게 **자원이 고아 상태로 남지 않도록 구조적으로 보장**(장비가 정상 응답한 실패 기준) · 벤더 추가 시 예외 타입·핸들러만 추가하는 확장 구조 확립',
          },
          {
            subtitle: '로드밸런서 공유 자원의 참조 관리 설계',
            body: [
              'Citrix 장비에서 백엔드 서버는 **{IP}_{PORT}로 식별되는 하나의 Service 객체**인데, 여러 개의 로드밸런서(가상서버)가 같은 백엔드를 공유해서 바인딩할 수 있다는 게 이 상품의 특성이었습니다. 문제는 **저희 애플리케이션 입장에서, 여러 가상서버가 이 Service를 안전하게 공유하려면 참조 상태를 직접 파악하고 있어야 했다**는 것이었습니다. VM을 연결할 때 이미 등록된 Service가 있으면 중복 생성하면 안 됐고, 연결을 해제할 때는 다른 가상서버가 아직 그 Service를 쓰고 있는지 모른 채 삭제하면 엉뚱한 서비스의 백엔드가 끊길 위험이 있었습니다.',
            ...
            result: '**여러 가상서버가 백엔드를 안전하게 공유할 수 있는 구조 확보** · 애플리케이션 레벨에서 참조 상태를 직접 관리해 중복 생성과 사용 중인 자원의 조기 삭제를 방지',
          },
          {
        ...
        achievements: [
          ...
          },
          {
            ...
            result: '**자원 고아 상태 구조적 방지**(장비 정상 응답 실패 기준) · 벤더 추가 시 예외 타입·핸들러만 추가하는 확장 구조',
          },
          {
            problem: 'Citrix 장비에서 백엔드는 **{IP}_{PORT}로 식별되는 하나의 Service 객체**인데 여러 로드밸런서(가상서버)가 같은 Service를 공유 바인딩 가능. **여러 가상서버가 안전하게 공유하려면 참조 상태를 애플리케이션에서 직접 파악해야 해** 중복 생성·사용 중인 자원의 조기 삭제 위험',
