        ...
        resumeContent: [
          ...
          },
          {
            ...
            body: [
              ...
              '중간 한 단계라도 실패하면 앞 단계에서 만든 자원이 그대로 남았습니다. 예를 들어 방화벽 계정은 만들어졌는데 VS 생성 단계에서 실패하면, 다음 시도에서 계정이 **"이미 존재한다"며 재시도조차 막혔습니다.** 이 흐름 전체가 **150~250줄짜리 한 서비스 메서드**에 몰려 있었고, 롤백도 단계마다 try-catch로 흩어져 있었습니다. 같은 문제는 포트포워딩처럼 여러 객체를 순서대로 만드는 다른 다단계 흐름에서도 반복됐습니다.',
              '**벤더 호출 순서를 조율하는 Integration 계층을 신설**했습니다. 순수 DB 로직(도메인 서비스)과 벤더 어댑터(external 서비스)를 분리하고, 엔티티 조립은 별도 Builder로 떼어냈습니다. 이렇게 흐름을 한 곳에 모으고 나니 프로비저닝 7단계(계정·VS·인터페이스·라우트·ACI·OpenStack·DB)마다 **실패 지점을 명확히 구분**할 수 있었습니다.',
              '이를 바탕으로 단계별로 롤백 범위가 다른 **11개 예외 타입 체계**를 설계했습니다. 분산 트랜잭션을 하나로 묶을 수 없는 환경에서 각 단계를 순차 실행하고, 실패 시 앞 단계를 보상 처리하는 **오케스트레이션 기반 Saga 패턴**입니다. 공통 예외는 상속으로 재사용하고, 하위 예외마다 롤백에 필요한 컨텍스트만 얹었습니다. 이 뼈대 위에서 **각 도메인 담당자는 자기 단계의 롤백만 정의하면 되도록** 만들어, 공통 구조와 도메인 지식을 분리했습니다.',
            ],
