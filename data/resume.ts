        ...
        resumeContent: [
          {
            ...
            body: [
              '당시 고객은 자기 자원을 직접 만들 수 없고, 신청하면 운영자가 대신 생성해주는 **매니지드 방식**이었습니다. 로드밸런서에 VM 하나를 연결하려 해도 작업 티켓 등록·운영자 확인·장비 직접 설정 절차를 거쳐야 해 **최소 1~2시간**은 걸렸습니다. 이런 반복 요청이 **체감상 월 50건 안팎**으로 운영자 손이 계속 묶였고, 고객 입장에선 자기 자원을 직접 제어할 수 없는 **제한적인 CSP 구조**였습니다.',
              'CSP 인프라 특성상 **장비 자체는 이미 멀티테넌트 기능을 갖추고 있었지만**, 이를 고객에게 안전하게 직접 열어주려면 **여러 벤더 장비를 안정적으로 오케스트레이션하는 문제**를 먼저 해결해야 했습니다. 그래서 이 기능을 운영자 개입 없이 고객에게 직접 안전하게 노출하는 API 서버를 신규 개발했습니다. 고객이 자기 테넌트 범위 안에서 직접 자원을 신청하면 즉시 처리되도록 흐름을 바꿨습니다.',
              '이 흐름 전환으로 고객은 방화벽·로드밸런서 자원을 직접 신청하고 제어할 수 있게 됐고, 운영자는 반복적인 수동 처리 업무에서 벗어날 수 있었습니다.',
            ],
            result: [
              '**최소 1~2시간** 걸리던 처리가 즉시 처리되는 구조로 전환',
              '**체감상 월 50건 안팎이던 반복 처리 요청이 한 자릿수 수준으로 감소**',
              '제한적이던 CSP 구조에서 **온전한 셀프서비스 환경**으로 전환, 운영자의 반복 업무 부담 해소',
            ],
          },
          {
            subtitle: 'Saga 패턴을 통한 프로비저닝 트랜잭션 보장',
            body: [
              '고객이 네트워크 환경을 하나 만들려면 방화벽 계정 생성부터 VS·인터페이스·라우트 설정, ACI 초기화, OpenStack 네트워크 생성까지 **서로 다른 벤더의 장비 API를 정해진 순서로** 호출해야 했습니다. 이 장비들은 전부 외부 시스템이라, **DB 트랜잭션은 물론 이를 자동화해주는 Spring @Transactional로도 묶을 수 없었습니다.**',
              '기존에는 이 호출 흐름과 예외 처리가 **하나의 서비스 메서드(약 200라인)에 강결합**돼 있어서, 중간 한 단계라도 실패하면 앞 단계에서 만든 자원이 그대로 남는 **자원 누수**가 발생했습니다. 예를 들어 방화벽 계정은 만들어졌는데 VS 생성 단계에서 실패하면, 다음 시도에서 계정이 **"이미 존재한다"며 재시도조차 막혔습니다.**',
              '순수 도메인 로직과 벤더 어댑터를 분리하는 **Integration 계층을 신설**해 프로비저닝 파이프라인을 구조화하고, 엔티티 조립은 별도 Builder로 떼어냈습니다. 이렇게 흐름을 한 곳에 모으고 나니 프로비저닝 7단계(계정·VS·인터페이스·라우트·ACI·OpenStack·DB)마다 **실패 지점을 명확히 구분**할 수 있었습니다.',
              '이를 바탕으로 단계별로 롤백 범위가 다른 **11개의 계층형 예외 타입**을 설계하고, **보상 트랜잭션 기반의 오케스트레이션(Saga 패턴)**을 구현했습니다. 공통 예외는 상속으로 재사용하고, 하위 예외마다 롤백에 필요한 컨텍스트만 얹었습니다. 이 뼈대 위에서 **각 도메인 담당자는 자기 단계의 롤백만 정의하면 되도록** 만들어, 공통 구조와 도메인 지식을 분리했습니다. **그 결과 새 벤더가 추가되거나 도메인 로직이 바뀌어도 기존 흐름을 건드리지 않고 안전하게 확장할 수 있었습니다.**',
            ],
            result: [
              '**7단계 프로비저닝 흐름에 11개 예외 타입 기반 차등 롤백 전략 설계**',
              '실패 지점과 무관하게 **자원 누수가 발생하지 않도록 구조적으로 보장**',
              '벤더 추가 시 예외 타입·핸들러만 추가하는 확장 구조 확립',
            ],
          },
          {
            subtitle: '로드밸런서 공유 자원의 참조 관리 설계',
            body: [
              'Citrix 장비에서 백엔드 서버는 **{IP}_{PORT}로 식별되는 하나의 Service 객체**인데, 여러 개의 로드밸런서가 같은 백엔드를 공유해서 바인딩할 수 있다는 게 이 상품의 특성이었습니다. 문제는 **여러 가상서버가 이 Service를 안전하게 공유하려면, 애플리케이션이 참조 상태를 직접 추적하고 있어야 했다**는 것이었습니다. VM을 연결할 때 이미 등록된 Service가 있으면 중복 생성하면 안 됐고, 연결을 해제할 때는 다른 가상서버가 아직 그 Service를 쓰고 있는지 모른 채 삭제하면 엉뚱한 서비스의 백엔드가 끊길 위험이 있었습니다.',
              '연결 시에는 **장비에 먼저 Service가 존재하는지 조회**해서, 있으면 타입을 확인하고 재사용하고 없으면 새로 생성하도록 처리했습니다. 해제 시에는 **DB에서 같은 IP·Port를 참조하는 다른 활성 바인딩이 있는지 쿼리로 확인**해서, 없을 때만 실제 장비에서 Service를 삭제하도록 만들어 **참조 카운팅**을 구현했습니다.',
            ],
            result: [
              '**여러 가상서버가 백엔드를 안전하게 공유할 수 있는 구조 확보**',
              '애플리케이션 레벨에서 참조 상태를 직접 관리해 중복 생성과 사용 중인 자원의 조기 삭제를 방지',
            ],
          },
          {
            ...
            body: [
              ...
              '다만 상품마다 특성이 달라 설계를 다르게 가져갔습니다. 방화벽은 고객 정보를 최상위에서 한 번만 연결하고 하위 데이터들이 연쇄적으로 연결되도록 구성해 데이터 중복을 줄였고, 로드밸런서는 우리 시스템에서 쓰는 고객 식별자와 실제 장비에서 쓰는 식별자가 서로 달라서, **두 값을 한 테이블에 함께 저장해 각자 필요한 곳에서 바로 꺼내 쓸 수 있게** 했습니다. 또한 데이터베이스뿐 아니라 실제 방화벽·로드밸런서 장비에서도 고객별로 구획을 나눠, 데이터와 장비 양쪽에서 이중으로 고객 간 정보가 섞이지 않도록 했습니다.',
            ],
            result: [
              '상품마다 다른 격리 요구사항을 하나의 데이터베이스 안에서 관리',
              '데이터베이스와 실제 장비 양쪽의 식별자를 함께 다뤄 처리 경로 단순화',
            ],
          },
        ],
        achievements: [
          ...
          },
          {
            ...
            analyze: '벤더 호출을 조율하는 **Integration 계층 분리** + 프로비저닝 7단계마다 **실패 지점별 롤백 범위가 다른 11개 예외 타입 체계**(오케스트레이션 기반 **Saga 패턴**)를 제안·구현 (도메인별 구체 롤백은 각 도메인 담당자가 구조 위에서 처리)',
      ...
      },
      {
        ...
        resumeContent: [
          ...
          },
          {
            ...
            body: [
              'RPMS는 공공기관 예산심사 시스템이라 **"누가 언제 무엇을 변경했는지" 추적 가능해야 하는 감사 요구사항**이 있었습니다. 그런데 단순히 "수정 요청이 오면 이력을 남긴다"로 구현하면, 실제 값 변경이 없는 저장 요청에도 **불필요한 이력이 과다 적재되어 감사 추적성을 오히려 저해하는** 문제가 생길 수 있다고 봤습니다.',
            ...
            result: [
              '값 변경이 있을 때만 이력을 남겨 심사자가 봐야 할 내역만 남도록 설계',
              '사업유형별로 검증 규칙을 분리해 실무자 입력 부담 축소',
            ],
          },
          {
            ...
            result: [
              '컴파일 단계에서 쿼리 오류 사전 감지',
              '지사 내 3개 프로젝트로 쿼리 방식 통일 확산',
            ],
          },
        ],
        achievements: [
