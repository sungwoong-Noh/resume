...
export const profile = {
  ...
  yearsOfExp: 5,
  tagline: '개발 표준과 공통 아키텍처를 직접 설계하고, 대용량 쿼리 성능을 99% 개선한 백엔드 개발자',
   ...
   */
  intro: `컨벤션·공통 모듈·에러 표준화를 리드 개발자로서 직접 수립하고, Spring Boot 기반 고성능 서버를 설계해왔습니다. 130만 건 DB p95를 17초에서 3초로, 동기화 배치를 2시간에서 10초로 단축한 것처럼 실행계획과 JPA 내부 동작을 이해해 근본 원인을 해결합니다.
MSA 환경에서 Saga 패턴 기반 보상 트랜잭션을 직접 설계하고, QueryDSL 도입·JUnit5 테스트 문화 정착 등 팀 전체 개발 생산성을 높이는 변화를 제안하고 이끌었습니다.
폐쇄망 Kubernetes 클러스터 운영부터 AWS ECS 기반 MSA 인프라 설계까지, 서비스 전체를 이해하는 시각으로 안정적인 플랫폼을 만들어가고 싶습니다.`,
}
